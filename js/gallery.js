...
class GalleryManager {
    ...
    if (this.activeItems.length === 0) {
      grid.innerHTML = `
        <div style="grid-column: 1 / -1; text-align: center; padding: 4rem 1rem; color: var(--text-muted);">
          ...
          <h3 style="color: var(--text-primary); margin-bottom: 0.5rem;">No Photos Found</h3>
          <p>No photos match your filter or search query. Try choosing another category.</p>
        </div>
      `;
      return;
    }
    ...
    grid.innerHTML = this.activeItems.map((item, index) => `
      <article class="gallery-card" onclick="window.gallery.openLightbox(${index})" tabindex="0" role="button" aria-label="View ${item.title}">
        <div class="gallery-thumb-container">
          ...
          <img src="${item.imageUrl}" alt="${item.title}" class="gallery-thumb" loading="lazy" onerror="this.src='https://images.unsplash.com/photo-1542816417-0983c9c9ad53?auto=format&fit=crop&w=800&q=80'">
  ...
  updateLightboxContent() {
    ...
    if (counter) counter.textContent = `Image ${this.lightboxIndex + 1} of ${this.activeItems.length}`;
